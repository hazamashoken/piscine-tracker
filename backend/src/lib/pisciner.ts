...
import { fetchLocationStats, fetchPisciner, fetchProject, fetchTeams, fetchUserCorrected } from '../api/fetches.js';
...
export async function processPisciner() {
  ...
  try {
    logger.info("Sending batch for pisciner");
    await batch.send();
  } catch (error) {
    logger.error(error);
  }
